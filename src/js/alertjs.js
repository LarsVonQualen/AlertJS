...
var AlertJS = (function (alertjs) {
    ...
    alertjs.setSetting = function (key, setting) {
        settings[key] = setting;
    };
    
    alertjs.getSetting = function (key) {
        return settings[key];
    };
    ...
    var PlaySound = function (sound) {
        if (settings.sounds !== {} && sound != undefined) {
            ...
            if (sound.mp3 != undefined) {
                player.append($("<source />").attr("src", sound.mp3).attr("type", "audio/mp3"));
            }
            
            if (sound.ogg != undefined) {
                player.append($("<source />").attr("src", sound.ogg).attr("type", "audio/ogg"));
            }
            
            /*if (sound.wav != undefined) {
                player.append($("<embed />").attr("hidden", "true").attr("autostart", "true").attr("loop", "false").attr("src", sound.wav));
            }*/
    ...
    alertjs.Notify = (function (notify) {
        ...
        var generateNotification = function (title, message, type, sound) {
            ...
            if (settings.sound == true || sound == true) { 
                PlaySound(settings.sounds[type]);
            }
